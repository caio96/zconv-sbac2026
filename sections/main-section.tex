...
To provide this support, \algname includes two versions: a zero-copy implementation that covers most common configurations (\rsec{zconv-alg}), and an extended version that introduces a small copy step to support grouped and dilated convolutions (\rsec{zconv-alg-ext}).
...
GEMM is called in row-major layout, with input matrices used as-is (\ie, no transposition), and with scalar multipliers \emph{alpha} and \emph{beta} both set to one.
...
A full implementation with padding handling is provided in the artifact~\cite{zconvartifact}.
  ...
  \For{$ow \gets 0$ to $\texttt{OW}-1$} \Comment{Parallel loop} \label{alg:zconv:ow}
    \For{$oh \gets 0$ to $\texttt{OH}-1$}   \label{alg:zconv:initialize-start}
      \For{$oc \gets 0$ to $\texttt{OC}-1$}
        \State $output[bt,oh,ow,oc] \gets bias[oc]$
      \EndFor
    \EndFor  \label{alg:zconv:initialize-end}

    \For{$fh \gets 0$ to $\texttt{FH}-1$}  \label{alg:zconv:fh}
      \State $A \gets \text{addr}(image[bt, fh, ow \cdot \texttt{SW}, 0])$ \label{alg:zconv:params-start}
      \State $B \gets \text{addr}(filter[fh,0,0,0])$
      \State $C \gets \text{addr}(output[bt,0,ow,0])$
...
\rfig{zconv-simple} illustrates an example of the \algname algorithm with the convolution parameters detailed in its caption.
...
Matrix $A$ is a non-contiguous tile, where each row contains $\texttt{FW}\times\texttt{IC}$ contiguous elements, and the leading dimension \emph{lda} specifies the stride between rows.
...
Matrix $C$ is stored as a non-contiguous tile (using the leading dimension \emph{ldc}) to avoid this transposed layout.
