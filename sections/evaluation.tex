...
Additional repetitions are done when 95\% confidence intervals (CI) overlap between two compared points and differences exceeded 1\%.
...
\begin{figure}[t]
    ...
    \caption{Scaling speedup relative to single-core execution for all layers from \rtab{perf-layers}. Dashed line marks ideal linear scaling.}
...
\begin{table}[t]
    \caption{Peak memory ratio (Baseline/\algname). Values above 1 indicate \algname uses less memory, bolded accordingly. Yaconv entries marked {---} are omitted because Yaconv produces incorrect results for those layers (3--4) or does not support the configuration (5--6).}
    \label{tab:memory}
    \small
