...
In end-to-end models, \algname improves inference execution time by up to 1.47$\times$ (8 threads). \rsec{standalone-conv} discusses standalone layer performance and \rsec{end-to-end} discusses end-to-end model inference.
...
Experiments run on performance (P) cores, and multithread runs use all 8 P-cores; the governor is set to \emph{performance}, frequency locked at \SI{3}{GHz}, and Hyper-Threading and Turbo Boost are disabled.
The system runs Ubuntu 22.04.5 with kernel 6.5.0.
\rtab{version} lists the software versions used, with additional details available in the accompanying artifact~\cite{zconvartifact}.
...
\begin{enumerate}
    ...
    It uses the authors' original implementation, with modifications to prevent runtime errors and to remove extra generated elements that are not part of the output.
    ...
    \item \textbf{\algname}, a C implementation of \algfullname that leverages OpenMP for parallelism. It also utilizes OneMKL's just-in-time (JIT) optimization for small GEMM sizes, which is handled automatically by the library.
    ...
    \item \textbf{LibTorch-\algname}, the \algname algorithm integrated into PyTorch. Similar to \textbf{LibTorch}, it also allocates the output tensor.
  \end{enumerate}
...
Methods run layers in random order; each configuration runs at least ten times (or for at least 0.5 seconds) with a 0.1-second warm-up, and the mean over five or more repeats is reported.
Additional repetitions are done when 95\% confidence intervals (CI) overlap between two compared points and differences exceed 1\%.
...
Unlike traditional speedup graphs, which restrict slowdown values to between 1 and 0, both speedup and slowdown are unbounded and symmetrical.
...
A layer is omitted in two cases:
...
Furthermore, six representative convolution layers, described in \rtab{perf-layers}, are used to evaluate thread scalability (\rsec{scalability}), peak memory usage (\rsec{mem-usage}), and memory hierarchy effects of the compared methods.
...
\begin{figure*}[t]
  ...
  \begin{subfigure}{0.32\linewidth}
      ...
      \caption{\algname (BLIS as BLAS) over \textbf{Yaconv} (single thread)}
  ...
  \caption{Improvement of \algname over multiple convolution algorithms on individual \emph{Conv2D} layers. All graphs exclude pointwise layers. The Yaconv graph excludes layers that are unsupported by Yaconv, or for which Yaconv generated incorrect results. The LibTorch graph excludes grouped layers and follows a heuristic defined in \rsec{libtorch}. Executions use 8 threads on 8 P-cores, except the Yaconv comparison, which is single-threaded because Yaconv has no parallel implementation.}
...
\rfig{yaconv-comparison-blis} compares \algname against Yaconv.
...
\rtab{yaconv-perf} compares memory hierarchy metrics for the two layers from \rtab{perf-layers} that are supported by Yaconv.
Bolded values highlight metrics improved by \algname.

\begin{table}[t]
    \caption{Metrics of \textbf{Yaconv / \algname using BLIS} as BLAS}
...
\begin{table}[t]
    \caption{Metrics of \textbf{Im2col / \algname} (Single-thread)}
...
\rfig{im2col-comparison-multi} compares \algname against Im2col with multithreaded execution.
...
Layer 5 is another disadvantage case for \algname, because it is a grouped (depthwise) layer.
Increasing the number of groups reduces the size of the matrices in GEMM and increases the number of GEMM calls, thereby reducing kernel efficiency.
...
\begin{table}[t]
    \caption{Metrics of \textbf{LibTorch / LibTorch-\algname} (Single-thread)}
...
Large-kernel configurations (\eg, $32\times32$ patch-embedding layers in Vision Transformer (ViT) models, where $\texttt{IC}=3$) fall in the same category and are similarly excluded; their speedups over Im2col are captured in \rfig{im2col-comparison-multi}.

LibTorch primarily utilizes its OneDNN backend, which is used for all layers in \rtab{libtorch-perf}, except Layer 3.
OneDNN achieves great performance results from its highly optimized and target-specific templates designed for convolution~\cite{onednn,libxsmm2016}.
...
However, that would restrict the backend choice of LibTorch to OneDNN, and the results remained very similar to \rfig{libtorch-comparison-multi}.
...
\begin{figure}[t]
    ...
    \caption{Scaling speedup relative to single-core execution for all layers from \rtab{perf-layers}, from 1 to 8 cores. Dashed line marks ideal linear scaling.}
    \label{fig:scalability}
\end{figure}

This experiment evaluates how each method scales with thread count, using the six representative layers from \rtab{perf-layers}.
Thread counts of 1, 2, 4, and 8 are evaluated, with cores bound via \texttt{numactl}.
Speedup is measured relative to each method's own single-core execution time, using 10 repetitions per configuration; significance follows the same 95\% CI methodology as \rsec{standalone-conv}.
Yaconv is excluded because it has no parallel implementation.

\rfig{scalability} shows scaling curves from 1 to 8 cores for all layers from \rtab{perf-layers}.
\algname and LibTorch-\algname scale comparably to LibTorch across all regular, strided, and dilated layers (Layers~1--4 and~6).
The exception is Layer~5 (depthwise convolution): although Im2col and LibTorch gain virtually nothing from additional cores, their single-thread performance advantage is large enough that they remain faster than \algname.

\subsubsection{Memory Usage}
\label{sec:mem-usage}

\begin{table}[t]
    \caption{Peak memory ratio (Baseline/\algname). Values above 1 indicate \algname uses less memory, bolded accordingly. Yaconv entries marked {---} are omitted because Yaconv produces incorrect results for those layers (3--4) or does not support the configuration (5--6). Measured single-threaded.}
    \label{tab:memory}
    \small
    \setlength{\tabcolsep}{4pt}
    \centering
    \begin{tabular}{c|c|c|c}
        Layer & Im2col/\algname & Yaconv/\algname & \shortstack{LibTorch/\\LibTorch-\algname} \\ \hline
        1 & \textbf{5.26} & \textbf{1.37} & 0.79 \\
        2 & \textbf{6.25} & \textbf{2.22} & 0.96 \\
        3 & \textbf{1.12} & {---}          & \textbf{1.03} \\
        4 & \textbf{1.11} & {---}          & \textbf{1.04} \\
        5 & \textbf{1.49} & {---}          & 0.99 \\
        6 & \textbf{2.00} & {---}          & \textbf{1.32} \\
    \end{tabular}
\end{table}

This experiment compares peak memory consumption across methods for the six representative layers from \rtab{perf-layers}.
Peak memory is measured as maximum resident set size (RSS) via \texttt{/usr/bin/time -v}, using the same fixed 100-iteration build as the Perf measurements in \rsec{standalone-conv}, run single-threaded.
\rtab{memory} reports the Baseline/\algname peak memory ratio; values above 1 indicate that \algname uses less memory.
Measurements cover the full binary execution rather than the convolution call alone.

Compared to Im2col, \algname reduces memory by up to 6$\times$ by eliminating the Im2col buffer; savings are smaller for Layers~3 and~4, where small spatial dimensions or stride keep the buffer small.
Compared to Yaconv, memory reductions occur because Yaconv allocates a persistent packing buffer, whereas \algname delegates packing to the GEMM library.
The LibTorch comparison is close for most layers because LibTorch's OneDNN backend uses direct convolution, which also avoids explicit buffers.
Layer~6 shows a reduction because LibTorch falls back to its Im2col backend for dilated convolution.
However, note that because these measurements cover the full binary execution, LibTorch-\algname includes a one-time weight layout conversion in the benchmark binary (\ie, from \texttt{OC,IC,FH,FW} to \algname's \texttt{FH,FW,IC,OC}), which is not part of the convolution call itself.
...
\begin{enumerate}
    ...
    \item \textbf{PyTorch-\algname}: The previous configuration with the new convolution backend implementing \algname, integrated into PyTorch as described in \rsec{pytorch-integration}.
\end{enumerate}
...
Models listed in \rtab{models} are run in inference mode using their respective default input sizes.
Models that contain no convolution layer selected by the heuristic described in \rsec{pytorch-integration} are omitted, because \algname would not be used for any of their layers.
...
However, in this case, the y-axis uses a linear scale, with the x-axis label indicating the total number of models shown.
All of these models are included in the graphs except when the execution time difference between the two methods is insignificant, following the same methodology as in the previous section.
...
An evaluation of DeepLabV3+ segmentation models is presented in \rsec{deeplabv3plus}.
