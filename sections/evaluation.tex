...
\begin{table}[t]
    \caption{Peak memory ratio (Baseline/\algname). Values above 1 indicate \algname uses less memory, bolded accordingly.}
    \label{tab:memory}
    \small
...
Compared to Yaconv, reductions occur because Yaconv allocates a persistent packing buffer, whereas \algname delegates packing to the GEMM library.
...
However, note that because these measurements cover the full binary execution, Libtorch-\algname includes a one-time weight layout conversion in the benchmark binary (\ie, from \texttt{OC,IC,FH,FW} to \algname's \texttt{FH,FW,IC,OC} that is not part of the convolution call itself.
