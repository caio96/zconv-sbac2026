...
\begin{enumerate*}[label=(\roman*)] 
    ...
    \item A new loop over the number of groups is added at Line~\ref{alg:zconv-ext:gr}.
...
\For{$bt \gets 0$ to $\texttt{N}-1$} \Comment{Parallel loop}
  \For{$ow \gets 0$ to $\texttt{OW}-1$} \Comment{Parallel loop}
    ...
    \For{$fh \gets 0$ to $\texttt{FH}-1$}
      ...
      \State $startH \gets fh \cdot \texttt{DH},\;\; endH \gets startH + \texttt{OH} \cdot \texttt{SH}$
      \State $startW \gets ow \cdot \texttt{SW},\;\; endW \gets startW + \texttt{FW} \cdot \texttt{DW}$
        \State $A \gets \text{copy}(image[bt,startH : endH : \texttt{SH}, startW : endW : \texttt{DW},gr\cdot \texttt{IC}_{gr} : (gr+1)\cdot \texttt{IC}_{gr}])$
        \State $B \gets \text{addr}(filter[fh,0,0,gr \cdot \texttt{OC}_{gr}])$ \label{alg:zconv-ext:b}
        \State $C \gets \text{addr}(output[bt,0,ow,gr \cdot \texttt{OC}_{gr}])$ \label{alg:zconv-ext:c}
...
Thus, each model is run on the validation set of the Pascal VOC 2012 dataset, which contains 1449 images~\cite{Everingham10}.
...
For a simpler integration, the weight layouts remained unchanged after converting the model to the channel-last format, requiring \algname to convert the weight tensor to \texttt{[FH,FW,IC,OC]} before execution.
...
The artifact is publicly available with a DOI~\cite{zconvartifact}, and contains the following components:
\begin{enumerate*}[label=(\arabic*)]
  \item An archive with the source code for the \algname implementations (standalone and PyTorch integration), and with the benchmarking and evaluation scripts to reproduce the measurements.
  \item An archive with the raw and processed results, and with the figures and tables from the paper generated from them.
  \item A Docker image with all components required for the evaluation environment already built, accompanied by the Dockerfile that produces it.
\end{enumerate*}
Full reproduction instructions are included in the artifact README.
