...
SMM-Conv and Yaconv observe that implicit smaller matrix multiplications arise when isolating a filter row (or column); these can be executed as GEMM operations.
...
\begin{enumerate*}[label=(\roman*)]
    ...
    \item Yaconv, SMM-Conv, and MEC do not fully support dilated, grouped, and strided convolutions.
    ...
    \algname is a more portable approach, relying entirely on GEMM kernels for optimizations and compatible with any BLAS library.
    ...
    \item Lastly, only \algname is evaluated on a large set of convolution layers, which enables the design of a heuristic to integrate it into a hybrid convolution approach, such as that used by PyTorch (\rsec{libtorch}).
\end{enumerate*}
...
\citet{Dukhan2019} introduced the Indirect Convolution, which modifies the GEMM routine used in an Im2col implementation to access the image tensor directly via an indirection buffer.
