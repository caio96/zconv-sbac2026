...
No single method is optimal for all convolution layers, as hybrid strategies in deep learning (DL) frameworks show.
...
Direct and General matrix-matrix multiplication (GEMM)-based convolutions are the two most widely used implementations, both supported by state-of-the-art DL frameworks such as PyTorch~\cite{pytorch2024} and ONNX Runtime~\cite{onnxruntime}.
...
These kernels are widely available across hardware platforms through Basic Linear Algebra Subprograms (BLAS) libraries~\cite{blis2015,openblas2013,Goto2008}.
...
Its core insight is that convolution can be decomposed into multiple GEMM calls that directly access the image tensor by exploiting the leading dimension parameters (\emph{lda}, \emph{ldb}, and \emph{ldc}) accepted by BLAS libraries.
...
Together, these algorithms cover most convolution configurations.
...
\begin{enumerate}[label=(\roman*),noitemsep,topsep=0pt] 
    ...
    \item An extensive evaluation of \algname against prior and state-of-the-art methods on standalone layers (\rsec{standalone-conv}).
    ...
    \item A publicly available artifact containing all implementations, evaluation scripts, and results~\cite{zconvartifact}.
\end{enumerate}
